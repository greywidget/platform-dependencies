...
def _len_of_title(book):
    return len(attrgetter("title")(book))


def _first_auth_last_name(book):
    authors = attrgetter("authors")(book)
...
def sort_books_by_len_of_title(books=books):
    ...
    """
    return sorted(books, key=_len_of_title)
...
def sort_books_by_first_authors_last_name(books=books):
    ...
    Automate the Boring Stuff with Python
    """
    return sorted(books, key=_first_auth_last_name)
